...
public class PreRetrieveGUI extends JFrame implements ActionListener
{
	...
	private RobustInformation info;

	public PreRetrieveGUI(RobustInformation info, boolean operation)
	{
		...
		lblVersion= new MyJLabel("Robust Data Hiding Used", arialFont, Color.black, Color.gray);
		...
		if(features == SecureReversible_ImageDataHiding.UUM || features== SecureReversible_ImageDataHiding.UEM
				|| features == SecureReversible_ImageDataHiding.CUM || features== SecureReversible_ImageDataHiding.CEM)
		...
		if(features == SecureReversible_ImageDataHiding.CUM || features== SecureReversible_ImageDataHiding.CUF
				|| features == SecureReversible_ImageDataHiding.CEM || features== SecureReversible_ImageDataHiding.CEF)
		...
		if(features== SecureReversible_ImageDataHiding.UEM || features== SecureReversible_ImageDataHiding.CEM
				|| features== SecureReversible_ImageDataHiding.UEF || features== SecureReversible_ImageDataHiding.CEF)
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else
		{
			...
			if(lblEncryptedStatus.getText().equals("YES"))
			{
				...
				message[1]= new JLabel("Please Enter Secret Key to Continue.");
				...
				do
				{
					...
					if(password.length()<8)
						JOptionPane.showMessageDialog(null, "Length of secret key should be atleast 8 Characters", "Invalid secret key", JOptionPane.OK_OPTION);
			...
			{
				String message= SecureReversible_ImageDataHiding.retrieveMessage(info, password);
				if(message!=null && !message.equals("#FAILED#"))
					new MessageDisplay(message, info.getFile().getName());
				else
				{
					message= SecureReversible_ImageDataHiding.getMessage();
					if(message!= null && message.equals("Incorrent Secret Key"))
						JOptionPane.showMessageDialog(null, "Incorrect Secret Key Specified!\nMake sure that Caps lock is not on.", "Invalid secret key!", JOptionPane.WARNING_MESSAGE);
					else 
						JOptionPane.showMessageDialog(null, "Error!\n"+ SecureReversible_ImageDataHiding.getMessage(), "Oops, Error!", JOptionPane.ERROR_MESSAGE);
				}
			}
			else
			{
				boolean res= SecureReversible_ImageDataHiding.retrieveFile(info, password, false);
				if(!res)
				{
					String message= SecureReversible_ImageDataHiding.getMessage();
					if(message!= null)
						if(message.equals("Incorrent Secret Key"))
							JOptionPane.showMessageDialog(null, "Incorrect secret key Specified!\nMake sure that Caps lock is not on.", "Invalid Secret Key!", JOptionPane.WARNING_MESSAGE);
							 ...
							 {							
								...
								SecureReversible_ImageDataHiding.retrieveFile(info, password, true);
								showFile(info);
							 }
							 else
								JOptionPane.showMessageDialog(null, "Error!\n"+ SecureReversible_ImageDataHiding.getMessage(), "Oops, Error!", JOptionPane.ERROR_MESSAGE);
	...
	private void showFile(RobustInformation info)
	{
...
class MessageDisplay extends JFrame
{
	...
	{
		super("Retrieved message from file '"+ fileName+ "' - Secure Reversible Image Data Hiding "+ SecureReversible_ImageDataHiding.VERSION+ "    -- ");
		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
