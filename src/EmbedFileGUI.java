...
public class EmbedFileGUI extends JFrame implements ActionListener, ItemListener
{
	...
	{
		super("Hiding file - Secure Reversible Image Data Hiding "+ SecureReversible_ImageDataHiding.VERSION+ "   -- ");
		...
		lblPassword= new MyJLabel("Secret Key (Minimum 8 chars)", arialFont, Color.black, Color.lightGray);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(source== btnGo)
		{
			...
			if(chkEncrypt.isSelected())
			{
				...
				{
					JOptionPane.showMessageDialog(this, "Secret key needs to be a minimum of 8 Characters!", "Invalid Secret Key!", JOptionPane.WARNING_MESSAGE);
					return;
				}
			}
			...
			if(SecureReversible_ImageDataHiding.embedFile(client.getMasterFile(), client.getOutputFile(), client.getDataFile(), compression, password))
				JOptionPane.showMessageDialog(this, SecureReversible_ImageDataHiding.getMessage(), "Operation Successful", JOptionPane.INFORMATION_MESSAGE);
			else
				JOptionPane.showMessageDialog(this, SecureReversible_ImageDataHiding.getMessage(), "Operation Unsuccessful", JOptionPane.ERROR_MESSAGE);
		}
	}
