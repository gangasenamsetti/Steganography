...
public class MainClient extends WindowAdapter implements ActionListener
{
	...
	private MainClient()
	{
		mainFrame= new JFrame(""+ SecureReversible_ImageDataHiding.VERSION+ " ");
		mainFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		mainFrame.addWindowListener(this);
		// Setup the menu bar
		mnuExit= new MyJMenuItem("Exit", 1, 'x');
		mnuEmbedMessage= new MyJMenuItem("Hide Message", 6, 'm');
		mnuEmbedFile= new MyJMenuItem("Hide File", 7, 'i');
		mnuRetrieveMessage= new MyJMenuItem("Retrieve Message(Reversible Image Data Hiding)", 0, 'r');
		mnuRetrieveFile= new MyJMenuItem("Retrieve File(Reversible Image Data Hiding)", 2, 't');
		mnuModifyMaster= new MyJMenuItem("Modify Master file settings", 2, 'd');
		mnuHelp= new MyJMenuItem("Help", 0, 'h');
		mnuAbout= new MyJMenuItem("Secure Reversible Image Data Hiding", 0, 'a');
		...
		ImageIcon im = new ImageIcon(this.getClass().getResource("Banner.png"));
		
		image = new JLabel();
	         image.setIcon(im);
		
		image.setBounds(20,30,860,490);
		...
		lblName= new MyJLabel("By:-   XYZ", myFont, Color.blue, Color.white);
		lblEmail= new MyJLabel("tmksmanju14@gmail.com", arialFont, myColor, Color.white);
		lblPhone= new JLabel(" ");
		...
		panelButtons.setBorder(new TitledBorder("File Embedding,Retrieving "));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(source== mnuAbout || source== btnAbout)
			SecureReversible_ImageDataHiding.showAboutDialog();

		if(source== mnuExit)
		{
			int result= JOptionPane.showConfirmDialog(mainFrame, "Are U Sure Want To Close.", "Confirm Exit", JOptionPane.YES_NO_OPTION);
			if(result== JOptionPane.YES_OPTION)
			{
				JOptionPane.showMessageDialog(mainFrame, "Welcome", "From Secure Reversible Image Data Hiding over Encrypted Domain via Key Modulation", JOptionPane.INFORMATION_MESSAGE);
				System.exit(0);
			}
		}
	}
	...
	{
		JOptionPane.showMessageDialog(mainFrame, "WELCOME", "From the Developer of TMKS Infotech", JOptionPane.INFORMATION_MESSAGE);
	}
	...
	private class RadioListener implements ItemListener
	{
		public void itemStateChanged(ItemEvent e)
		{
			...
			try
			{
				...
				SecureReversible_ImageDataHiding.updateUserInterface();
			}
			catch(Exception ex)
			{
