...
public class SecureReversible_ImageDataHiding {
	...
	private SecureReversible_ImageDataHiding() {
		System.out.println("Secure Reversible Image Data Hiding " + VERSION + " ready...");
	}
	...
	public static boolean embedMessage(File masterFile, File outputFile,
		...
		if (password != null && password.length() < 8) {
			message = "Secret Key should be minimum of 8 Characters";
			return false;
		}
		...
		message = "Message Hidden successfully in file '"
				+ outputFile.getName() + "'.";
		return true;
	}

	// Retrieves an embedded message from a Master file
	public static String retrieveMessage(RobustInformation info,
			String password) {
		String messg = null;
		...
		try {
			...
			messg = new String(RobustInformation.byteToCharArray(messageArray));
		} catch (Exception e) {
			message = "Oops!!\n Error: " + e;
			e.printStackTrace();
			return ("#FAILED#");
	...
	public static boolean embedFile(File masterFile, File outputFile,
		...
		message = "File '" + dataFile.getName()
				+ "' Data Hidden successfully in file '" + outputFile.getName()
				+ "'.";
		return true;
	}

	// Retrieves an embedded file from a Master file
	public static boolean retrieveFile(RobustInformation info,
		...
		try {
			...
			if (messageSize <= 0) {
				message = "Unexpected size of Hidden file: 0.";
				return false;
			}
	...
	private static class AboutFrame extends JFrame implements ActionListener {
		...
		public AboutFrame() {
			setTitle("About Secure Reversible Image Data Hiding " + SecureReversible_ImageDataHiding.VERSION);
			filler1 = new JLabel(" ");
			filler2 = new JLabel(" ");
			filler3 = new JLabel(" ");
			lblTitle = new MyJLabel("Secure Reversible Image Data Hiding over Encrypted Domain via Key Modulation "
					+ SecureReversible_ImageDataHiding.VERSION, new Font("Arial", Font.PLAIN,
					15), YELLOW, BLACK);
			lblName = new MyJLabel("By:  TMKS Infotech,Bangalore", new Font("Monotype Corsiva",
					Font.PLAIN, 14), GREEN, BLACK);
			lblEmail = new MyJLabel("      tmksmanju14@gmail.com", GREEN, BLACK);
			lblPhone = new MyJLabel("      +91 9535866270", GREEN, BLACK);
			if (new File("Images/About").exists())
				lblImage = new JLabel(new ImageIcon("Images/Myself"));
			else {
				lblImage = new MyJLabel("Secure Reversible Image Data Hiding over Encrypted Domain via Key Modulation "
						+ SecureReversible_ImageDataHiding.VERSION, new Font("Times new roman",
						Font.PLAIN, 50), Color.red, Color.black);
			...
			panelAbout = UtilityOperations.createBorderedPanel(panelAbout,
					"About Secure Reversible Image Data Hiding", 3, 3);
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnVisit)
				new HTMLFrame("http://www.tmksinfotech.com", true);
...
class RobustInformation {
	...
	public RobustInformation(File file) {
		this.file = file;
		isEster = false;
		...
		if (fileExtension.equalsIgnoreCase("jpg"))
			inputMarker = SecureReversible_ImageDataHiding.OFFSET_JPG;
		else if (fileExtension.equalsIgnoreCase("png"))
			inputMarker = SecureReversible_ImageDataHiding.OFFSET_PNG;
		else
			inputMarker = SecureReversible_ImageDataHiding.OFFSET_GIF_BMP_TIF;
...
class HTMLFrame extends JFrame implements HyperlinkListener {
	...
	public HTMLFrame(String startURL, boolean isOnline) {
		super("Help - Robust Data Hiding " + SecureReversible_ImageDataHiding.VERSION
				+ " -- By: XYZ");
		...
		if (isOnline) {
			...
			setTitle("Robust Data Hiding " + SecureReversible_ImageDataHiding.VERSION
					+ " -- By: XYZ");
		}
		// else
